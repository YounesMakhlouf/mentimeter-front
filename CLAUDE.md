...
- Modals use `src/Components/Modal.tsx` — a thin wrapper over the native `<dialog>` element. `showModal()` gives us the focus trap, Escape handling, return-focus, `aria-modal`/`role="dialog"`, and the `::backdrop` layer for free. Backdrop clicks are detected by `e.target === dialogRef.current`. The subtree is conditionally rendered on `open` so form components re-init each time. `ModalBox` extends `Stack`, so children automatically get the right vertical rhythm.
